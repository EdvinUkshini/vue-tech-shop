            ...
            <template>
              <v-treeview :items="items" :open="[1]" :active="[5]" :selected-color="'#fff'" activatable open-on-click dense>
                    <template slot="label" slot-scope="{ item }">
                      <a @click="filterByCpu(item.value.split('(')[0])">{{ item.name }}</a>
                    </template>
              </v-treeview>
            </template>
            <v-divider></v-divider>
        ...
        >
          ...
          <v-row dense>
            <v-col cols="12" sm="8" class="pl-6 pt-6">
              <small>Showing {{ Laptops.length }} products</small>
            </v-col>
          ...
          <div class="row text-center">
            ...
            <div class="col-md-3 col-sm-6 col-xs-12" v-bind:key="item.name">
              <v-hover v-slot:default="{ hover }">
                ...
                <v-card-actions>
                  <router-link :to="`/details/laptop/${item._id}`" style="color:#00cc6a;">Details</router-link> 
                </v-card-actions>
                </v-card>
              </v-hover>
            </div>
