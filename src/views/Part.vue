        ...
        >
          ...
          <v-row dense>
            <v-col cols="12" sm="8" class="pl-6 pt-6">
              <small>Showing {{ Parts.length }} products</small>
            </v-col>
          ...
          <div class="row text-center">
            ...
            <div class="col-md-3 col-sm-6 col-xs-12" v-bind:key="item.name">
              <v-hover v-slot:default="{ hover }">
                ...
                <v-card-actions>
                  <router-link :to="`/details/part/${item._id}`" style="color:#00cc6a;">Details</router-link> 
                </v-card-actions>
                </v-card>
              </v-hover>
            </div>
...
import DataService from "../services/PartDataService";
    export default {
        data: () => ({
         ...
         methods: {
            retrieveParts() {
              DataService.getAll(this.$route.params.type,this.range[0],this.range[1])
                .then(response => {
